...
describe('testing LinkedList', function(){
  ...
  test('getNth should return the corect node', () => {
    let list = LinkedList.fromArray([1,2,3,4]);
    expect(list.getNth(0)).toEqual(list.head.value);
    expect(list.getNth(1)).toEqual(list.head.next.value);
    expect(list.getNth(2)).toEqual(list.head.next.next.value);
    expect(list.getNth(3)).toEqual(list.head.next.next.next.value);
  });
